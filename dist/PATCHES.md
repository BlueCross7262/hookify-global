...
- **변경 파일**: `core/config_loader.py`, `core/rule_engine.py`
- **의도**: 룰 frontmatter 에 `cwd_path_scope`(bool) 플래그를 추가한다. `cwd_path_scope: true` 면 bash·powershell 명령 인자에서 파일 경로를 추출해, 그 경로가 세션 cwd 안을 가리킬 때만 룰이 발동한다. cwd 밖만 가리키거나(예: `~/.claude/x`) 경로를 특정할 수 없으면(경로 토큰 없음, 예: `ls`·`python build.py`) skip 한다.
...
- **수용 기준**: 명령이 cwd 안 경로(`./src/x`·절대 in-cwd) 포함 → 발동. cwd 밖 절대경로만(`~/.claude/x`) → skip. 경로 토큰 없음(`ls`·`build.py`) → skip. 안+밖 혼합 → 발동(안 경로 존재). `cwd_path_scope` 부재 룰은 영향 없음(하위호환). `cwd_scope`(patch 08)와 독립이며 병용 가능(둘 다 AND).
- **한계(휴리스틱)**: 구분자(`/`·`\`·`~`) 있는 토큰만 경로로 본다. 따라서 cwd 루트의 구분자 없는 파일명(`rm config.json`)·`$VAR` 로 조립된 경로는 미감지(skip), `sed s/a/b/`·`echo a/b` 등 슬래시 포함 비경로 토큰은 과차단될 수 있다(URL `scheme://…` 은 예외 처리). 공백 포함 경로는 `split()` 로 분절될 수 있다. `command` 필드 없는 이벤트(`file`·Edit 등)에는 발동하지 않는다(bash·powershell 전용). ctx 유도 목적상 오판은 비치명적이며 정밀 분석엔 셸 토크나이저가 필요하다(범위 외).
