...
- **수용 기준**: PreToolUse 차단엔 `permissionDecisionReason` (중복 콘솔 출력 방지를 위해 `systemMessage` 미포함), Post·Stop·UserPromptSubmit 차단엔 top-level `reason`.
...
### 08-cwd-scope.patch (P1)
- **변경 파일**: `core/config_loader.py`, `core/rule_engine.py`
- **의도**: 룰 frontmatter 에 `cwd_scope`(bool)·`cwd_pattern`(정규식) 플래그를 추가한다. `cwd_scope: true` 이고 `cwd_pattern` 이 PreToolUse 훅 입력의 `cwd`(세션 작업 디렉터리)와 매칭될 때만 룰이 발동하고, 불일치 워크스페이스에서는 skip 한다. `cwd_scope` 미설정(기본 false) 시 기존처럼 전역 발동한다.
- **근거**: 전역 룰(`*.global.md`)을 특정 워크스페이스에서만 적용하기 위함. 기존에는 command 문자열의 경로를 negative lookahead 로 제외해야 해 대상 폴더가 늘면 관리가 불가능했다. 플래그 1개(`cwd_scope`)로 전역/워크스페이스 스코프를 전환한다.
- **수용 기준**: `cwd_scope: true` + `cwd_pattern` 일치 워크스페이스에서 룰 발동, cwd 불일치 워크스페이스에서 미발동. `cwd_scope` 부재 룰은 기존과 동일하게 전역 동작(하위호환). `cwd_pattern` 의 경로 구분자는 `[\\/]` 로 Windows(`\`)·POSIX(`/`) 모두 매칭한다.
- **fail-safe**: `cwd_scope: true` 인데 `cwd_pattern` 이 비면(오타·누락) 전역 차단으로 새지 않도록 어디서도 발동하지 않는다. cwd 가 부재·`null` 이면 빈 문자열로 수렴해 미발동한다(`TypeError` 없음).

### 09-cwd-path-scope.patch (P1)
- **변경 파일**: `core/config_loader.py`, `core/rule_engine.py`
- **의도**: 룰 frontmatter 에 `cwd_path_scope`(bool) 플래그를 추가한다. `cwd_path_scope: true` 면 bash·powershell 명령 인자 또는 편집 도구(Edit·MultiEdit·Write)의 `file_path` 에서 파일 경로를 추출해, 그 경로가 세션 cwd 안을 가리킬 때만 룰이 발동한다. cwd 밖만 가리키거나(예: `~/.claude/x`) 경로를 특정할 수 없으면(경로 토큰 없음, 예: `ls`·`python build.py`) skip 한다.
- **근거**: 프로젝트 안 파일을 건드리는 shell 은 통제(ctx 유도)하되, 프로젝트 밖(`~/.claude` 등) 작업과 경로 없는 명령은 허용하기 위함. 기존 command 문자열 negative-lookahead 경로 제외 방식을 일반화한다. 임의 shell 문자열 파싱은 휴리스틱(구분자 `/`·`\`·`~` 토큰만 경로로 인식)이라 best-effort 이며, 오판 시 over/under-block 은 ctx 유도 목적상 치명적이지 않다.
- **수용 기준**: 명령이 cwd 안 경로(`./src/x`·절대 in-cwd) 포함 → 발동. cwd 밖 절대경로만(`~/.claude/x`) → skip. 경로 토큰 없음(`ls`·`build.py`) → skip. 안+밖 혼합 → 발동(안 경로 존재). 편집 도구는 `file_path` 가 cwd 안 → 발동, 밖 → skip(`command` 키 없어도 동작). `cwd_path_scope` 부재 룰은 영향 없음(하위호환). `cwd_scope`(patch 08)와 독립이며 병용 가능(둘 다 AND).
- **한계(휴리스틱)**: 구분자(`/`·`\`·`~`) 있는 토큰만 경로로 본다. 따라서 cwd 루트의 구분자 없는 파일명(`rm config.json`)·`$VAR` 로 조립된 경로는 미감지(skip), `sed s/a/b/`·`echo a/b` 등 슬래시 포함 비경로 토큰은 과차단될 수 있다(URL `scheme://…` 은 예외 처리). 공백 포함 경로는 `split()` 로 분절될 수 있다. `command` 와 `file_path` 둘 다 없는 도구(예: WebFetch)에는 발동하지 않는다. ctx 유도 목적상 오판은 비치명적이며 정밀 분석엔 셸 토크나이저가 필요하다(범위 외).
